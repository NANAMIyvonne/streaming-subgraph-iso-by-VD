...
int main(int argc, char** argv) {
    ...
    closedir(dirp);
    cout << "Got " << filenames.size() << " files input." << endl;
    ...
    for (Graph queryGraph : queryGraphVector) {
        ...
        if (arguments.useCache) {
            ...
            float use_time = std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::steady_clock::now() - sStart).count();
            cout << "Search Using Time: " << use_time/1000 << "ms" << endl;
            searchTime += use_time;
            ...
            float scan_time = std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::steady_clock::now() - sStart).count()/1000.0;
            totalScanTime += scan_time;
            float overhead = std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::steady_clock::now() - sStart).count();
            overHeadTime += overhead;
            cout << "Scan time: " << scan_time << " ms." << endl;
            cout << "Overhead: " << overhead/1000 << " ms." << endl;
            bool useCache = false;
            if (cachedGraphIndexCase1 != -1) {
                ...
                float curr_oh = std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::steady_clock::now() - sStart2).count();
                overHeadTime += curr_oh;
                overhead += curr_oh;
                ...
                std::chrono::steady_clock::time_point emb_start = std::chrono::steady_clock::now();
                std::vector<std::map<int,int>> * cachedEmbedding = graphCache.getEmbeddingsById(cachedGraphIndexCase2); 
                float emb_time = std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::steady_clock::now() - emb_start).count();
                cout << "Emb_time: " << emb_time/1000 << " ms." << endl;
                if (cachedEmbedding != NULL) {
                    hit ++;
            ...
            std::chrono::steady_clock::time_point no_cache_time = std::chrono::steady_clock::now();
            if (!useCache) {
            ...
            float not_using_cache_time = std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::steady_clock::now() - no_cache_time).count();
            cout << "Not using Cache overhead: " << not_using_cache_time/1000 << " ms." << endl;
            // graphCache.printCache();
            cout << "Over head time: " << overhead/1000 << " ms." << endl;
        } else {
            bool timedout = false;
            std::vector<std::map<int,int>> resultsGraph;
            if (arguments.vf2) {
    ...
    std::cout << "Total comparation: " << totalComparation << std::endl;
    std::cout << "Cache size: " << arguments.cache_size << std::endl;
