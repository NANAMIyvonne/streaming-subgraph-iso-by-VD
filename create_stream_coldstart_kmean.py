...
while k_round < 10:
    for max_subgraph_nodes in [10, 15, 20, 25, 30]:
        ...
        base_number = k_round * N_CLUSTERS
        ...
        for i, query_line in enumerate(dimas_query_lines):
            with open(os.path.join(savedir,  'query' , 'q{:04d}.dimas'.format(i + base_number)), 'w') as f:
                for line in query_line:
                    f.write(line)
        for i, query_line in enumerate(dimas_query_lines_weighted):
            with open(os.path.join(savedir,  'query_weighted' , 'q{:04d}.dimas'.format(i + base_number)), 'w') as f:
                for line in query_line:
                    f.write(line)
